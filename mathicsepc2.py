...
totalLossList2 = np.zeros((len(totalBiasList),len(totalWeightList),len(totalWeightList)))
...
for i in range((batch_id-1)*batch_size,(batch_id)*batch_size):
    for j in range(len(totalWeightList)):
        for k in range(len(totalWeightList)):
            l = l+1
            t = time.time()
            totalLossList2[i,j,k] = Interpol(30,2,100,torch.tensor(totalBiasList[i,:]),torch.tensor(totalWeightList[j,:]),torch.tensor(totalWeightList[k,:]),-1,1)
            print(f'Iteration: {l}/{totallen}, Machine: {batch_id}, Loss {totalLossList2[i,j,k]},Time : {time.time()-t}, Time remaining : {(totallen-l)*(time.time()-t)/3600:.2f} h ')

from tempfile import TemporaryFile
outfile = TemporaryFile()
np.save("bf2.npy",totalLossList2)