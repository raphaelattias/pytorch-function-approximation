...
totalWeightList = ListInitWeights(np.linspace(-1,1,6),3)
totalBiasList = ListInitWeights(np.linspace(-1,1,6),3)
...
for i in range((batch_id-1)*batch_size,(batch_id)*batch_size):
    for j in range(len(totalWeightList)):
        for k in range(len(totalWeightList)):
            l = l+1
            t = time.time()
            totalLossList[i,j,k] = Interpol(30,3,100,torch.tensor(totalBiasList[i,:]),torch.tensor(totalWeightList[j,:]),torch.tensor(totalWeightList[k,:]),-1,1)
