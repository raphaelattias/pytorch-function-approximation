...
totalWeightList = ListInitWeights(np.linspace(-1,1,5),2)
...
for i in range((batch_id-1)*batch_size,(batch_id)*batch_size):
    for j in range(len(totalWeightList)):
        for k in range(len(totalWeightList)):
            l = l+1
            t = time.time()
            totalLossList[i,j,k] = Interpol(30,2,50,torch.tensor(totalBiasList[i,:]),torch.tensor(totalWeightList[j,:]),torch.tensor(totalWeightList[k,:]),-1,1)
            #print(f'Weight {l}/{totallen}, Loss {totalLossList[i,j,k]}, IW IB OW: {totalWeightList[j,:]}, {totalBiasList[i,:]}, {totalBiasList[k,:]}, Time : {time.time()-t}, Time remaining : {(totallen-k)*(time.time()-t)/360:.2f} h')
            print(f'Iteration: {l}/{totallen}, Machine: {batch_id}, Loss {totalLossList[i,j,k]},Time : {time.time()-t}, Time remaining : {(totallen-l)*(time.time()-t)/3600:.2f} h ')
